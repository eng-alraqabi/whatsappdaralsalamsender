...
//var cont = require('./controller.js');
...
//     con.getAccounts(req, res);
//     console.log("Done ");
// });

// app.get('/Home/', function (req, res) {
//     //app.url = "/TransactionsView/";
//     app.router.length
//     app.use("/TransactionsView/", con.getAccounts(req, res))
// });
...
//     if(sent.ack>=0){
//         res.send('Message Sent'+sent.id.id);
//     }else{
//         res.send('Error');
//     }
// });
...
app.post('/SendSms',  async (req, res) => {
    ...
    var mony = req.body.mony;
    var note = req.body.note;
    var message_text = `*إشعار*
مرحبا ${name} نود إشعاركم انه تم تحرير في حسابكم لدينا التالي :-
    *عمــلية*: ${type}
    *بملـــغ*: ${mony} ريال يمني
    *بتاريـخ*: ${date}
    *البيـان*: ${note}
*وتقبلوا خالص تحياتنا مركز دار السلام النموذجي*`;
    var sent = await client.sendMessage(num, message_text);
    if (sent.ack >= 0) {
        res.send('Message_Sent_id :' + sent.id.id);
    } else {
      res.send('Error');
    }
// if (sent.ack >= 0) {
...
// var msg = await client.sendMessage(num, message_text);
// console.log('befor');
// var id=await msg.id.id;
// console.log(id);
// if ( msg.ack >= 0) {
//     res.send('Message_Sent_id :' + msg.id.id);
// } else {
//     res.send('Error');
// }
// });
//--------------------------------------------------------
app.post('/SendSmsBalance', async (req, res) => {
    var num = req.body.number.toString() + '@c.us';
    var name = req.body.name;
    var date = req.body.date;
    var mony = req.body.mony;
    var message_text = `*طلب سداد*
مرحبا عميلنا الكريم :  ${name} نود إشعاركم ان رصيد حسابكم لدينا التالي :-
    *عليــــكم*: ${mony} ريال يمني
    *حتى تاريخ*: ${date}
    ونرجوا منكم سرعة سداد ماعليكم
*وتقبلوا خالص تحياتنا مركز دار السلام النموذجي*`;
    var msg1 = await client.sendMessage(num, message_text);
    client.on('message_ack', (msg1, ack) => {
        if (ack >= 0) {
             res.send('Message_Sent_id :' + msg1.id.id);
        } else {
             res.send('Error');
        }
    });
...
//     if (msg.body.startsWith('السلام')) {
//         msg.reply('وعليكم السلام ورحمة الله وبركاتة');

//     } else if (msg.body.startsWith('من مع') || msg.body.startsWith('من انت')) {
//         client.sendMessage(msg.from, 'نحن مركز دار السلام النموذجي');

    
//     }else if (msg.body.startsWith('55555كشف حسابي؟')) {
//         var num = '' + (await msg.getContact()).number;
//         cont.showaccount(num, function (err, data) {
//             if (data != null) {
//                 var blance = data[0]['Debit'] - data[0]['Credit'];
//                 var state = 'عليكم';
//                 if (blance < 0) {
//                     state = 'لكم';
//                 }
//                 var message_text = `*كشف حساب اجمالي*
// مرحبا ${data[0]['Name']} كشف حسابكم لدينا هو:-
//     *عليكم* : ${data[0]['Debit']} 
//     *واصل لكم* : ${data[0]['Credit']} 
//     *رصيد حسابكم هو* : ${blance} 
//     *حالة الرصيد* : ${state}
// *وتقبلوا خالص تحياتنا مركز دار السلام النموذجي*`;
//                 client.sendMessage(msg.from, message_text);
//             } else {
//                 //client.sendMessage(msg.from, "المعذرة رقمكم غير مرتبط باي حساب لدينا يرجى كتابة اسمك الكامل لكي يتم التواصل معك وربط  رقمكم بحسابكم لدينا");
//                 msg.reply("المعذرة رقمكم غير مرتبط باي حساب لدينا يرجى كتابة اسمك الكامل لكي يتم التواصل معك وربط  رقمكم بحسابكم لدينا");
//             }


//         });

//     } else if (msg.body.startsWith('ربط حسابي')) {
//         let chat = await msg.getChat();
//         //chat.id
//         if (chat.isGroup) {
//             let newSubject = msg.body.slice(9);
...
//         let list = new List('List body', 'btnText', sections, 'Title', 'footer');
//         client.sendMessage(msg.from, list);
//     } else if (msg.body.startsWith('تسجيل')) {
//         msg.react('👍');
//     } else if (msg.body.startsWith('رقمي')) {
//         msg.reply('' + (await msg.getContact()).number);
//     }
